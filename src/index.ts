...
export class GraphQLClient {
  ...
  async query<T extends any>(
    ...
    let bodyString = 'query=' + encodeURIComponent(query)

    if (variables) {
      bodyString += '&variables=' + encodeURIComponent(JSON.stringify(variables ? variables : undefined))
    }

    const response = await fetch(this.url + '?' + bodyString, {
      method: 'GET',
      headers: Object.assign({ 'Content-Type': 'application/json' }, headers),
      ...others,
    })
